...
def convert_csv_to_yaml(csv_path, yaml_path):
    ...
    column_order = [
        'title', 'year', 'id', 'name', 'link', 'deadline', 'abstract_deadline', 'timezone', 'place', 'date',
        'start', 'end', 'paperslink', 'pwclink', 'hindex', 'sub', 'ranking', 'note', 'wikicfp'
    ]
    data = data[[name for name in column_order if name in data.columns]]  # sort by specified order
